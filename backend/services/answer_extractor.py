...
# Reason: Generic, domain-agnostic prompt for handwritten answer segment extraction and 2D spatial grounding
ANSWER_PROMPT = """
...
Guidelines:
1. Detect all question markers or answer identifiers on the page (e.g., circled numbers like (4), (5), (6), (7), (9), (10), margin indicators, prefixes like Q1, Ans 2).
2. For every distinct answer, determine the precise 2D bounding box covering the question marker AND the entire handwritten response:
   - Provide "box_2d": [ymin, xmin, ymax, xmax] in normalized integer coordinates from 0 to 1000.
...
# Reason: Extract raw answer segments with Gemini Vision 2D spatial grounding
def extract_answers_from_page(page_img: Image.Image, page_num: int) -> List[AnswerSegment]:
    for model_name in FALLBACK_MODELS:
        try:
            print(f"[GEMINI VISION] Extracting answers on page {page_num} using model: {model_name}")
            ...
            if segments:
                print(f"[GEMINI VISION SUCCESS] Model {model_name} extracted {len(segments)} answers on page {page_num}.")
                return segments
        except Exception as e:
            print(f"[GEMINI VISION ERROR] Page {page_num} with model {model_name} failed: {e}")
